...
namespace Автошкола
{
    public class CarriersDA
    {
        ...
        public void Save(AutoschoolDataSet dataSet, AbstractConnection conn, AbstractTransaction tr)
        {
            ...
            // на вставку 
            dataAdapter.InsertCommand = new SqlCommand("INSERT INTO Carriers (ID, Brand, Model, StateNumber, " +
                "Color, Transmission, Category, Status)  VALUES (@ID, @Brand, @Model, @StateNumber, " +
                "@Color, @Transmission, @Category, @Status)", conn.getConnection(), tr.getTransaction());
        ...
        public void ReadByCategory(AutoschoolDataSet dataSet, AbstractConnection conn, AbstractTransaction tr, int CategoryID)
        {
            dataAdapter = new SqlDataAdapter();
            dataAdapter.SelectCommand = new SqlCommand("SELECT * FROM Carriers WHERE Category = @CategoryID", conn.getConnection(), tr.getTransaction());
